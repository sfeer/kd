...
var __WEBPACK_AMD_DEFINE_FACTORY__, __WEBPACK_AMD_DEFINE_ARRAY__, __WEBPACK_AMD_DEFINE_RESULT__;var _typeof = (__webpack_require__(7847)["default"]);
...
;// CONCATENATED MODULE: ./packages/menu.ts
...
var KdMenu = /*#__PURE__*/function () {
  // 高亮菜单
  // 开启菜单
  // 当前菜单id
  function KdMenu(menus, path) {
    var _this = this;

    _classCallCheck(this, KdMenu);

    _defineProperty(this, "path", '/');

    _defineProperty(this, "activeMenu", []);

    _defineProperty(this, "openMenu", []);

    _defineProperty(this, "cid", undefined);

    _defineProperty(this, "idMap", new Map());

    _defineProperty(this, "urlMap", new Map());

    this.setPath(path);
    this.loop(menus, function (v) {
      _this.idMap.set(v.id, v);

      v.url && _this.urlMap.set(v.url, v);
    });
  } // 正向递归
  ...
  _createClass(KdMenu, [{
    key: "loop",
    value: function loop(menus, handle) {
      var _this2 = this;
    ...
    key: "loop2",
    value: function loop2(menu, handle) {
      handle(menu);

      if (menu.pid) {
        var p = this.idMap.get(menu.pid);
    ...
    value: function setPath(path) {
      ...
      if (path) {
        this.path = path;
        var one = this.urlMap.get(path);

        if (one) {
          this.cid = one.id;
          this.loop2(one, function (v) {
            _this3.activeMenu.push(v.id);

            if (v.pid) {
              _this3.openMenu.push(v.pid);
            }
          });
        }
      } else {
