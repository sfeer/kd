<template>
  <a-form-item :label="model.name">
    <a-input v-if="type === 'input'" v-model:value="vv" :placeholder="`请输入${model.name}`" v-bind="model.props" />
    ...
    <a-date-picker
      ...
      v-model="vv"
    />
    <a-textarea v-else-if="type === 'textarea'" :placeholder="`请输入${model.name}`" :auto-size="{ minRows: 2 }" />
